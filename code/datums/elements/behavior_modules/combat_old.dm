...
/datum/behavior_module/combat/proc/designate_target(datum/source, list/potential_targets)
	to_chat(world, "designating target time")
	var/is_type
	...
	if(length(potential_targets)) //We got baddies to kill, signify stance change and pick out a target
		SEND_SIGNAL(source_holder.parent, COMSIG_AI_ATTEMPT_CHANGE_STANCE, AI_ATTACKING, 2)
		SEND_SIGNAL(source_holder.parent, COMSIG_SET_AI_MOVE_TARGET, pick(potential_targets))
