	...
	switch(stance)
		if(AI_ATTACKING)
			for(var/signal in typepaths_to_trigger[source])
				UnregisterSignal(source, signal, .proc/designate_target)
...
/datum/element/behavior_module/combat/proc/designate_target(datum/source, atom/the_target)
	if(SEND_SIGNAL(source, COMSIG_AI_ATTEMPT_CHANGE_STANCE, AI_ATTACKING, 2))
		targets[source] = the_target
		RegisterSignal(the_target, list(), .proc/undesignate_target, the_target)
...
/datum/element/behavior_module/combat/proc/undesignate_target(datum/source, atom/the_target)
	if(the_target)
		UnregisterSignal(the_target, list())
